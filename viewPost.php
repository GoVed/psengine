...
<body>	
	<?php
		...
		echo '<div class="topnav"><span style="display:inline-block;margin:10px;"><big>'.$row['title'].'</big><small> '.timeDiff($row['id'],round(microtime(true) * 1000)).' ago by <span class="clickable" onclick="location.href=\'profile.php?username='.$row['username'].'\'">'.$row['username'].'</span></small></span><a href="index.php">Home</a>';
		if(array_key_exists("username", $_SESSION)){
			if($_SESSION["username"]==$row["username"]){
				echo '<a id="removeButton" onclick="remove(\''.$_REQUEST["id"].'\')">Remove</a>';
			}
		}
		echo '</div>';
	?>
	<div style="padding-bottom:35px;">This is how I align elements, XD</div>
	<div class="data_root">
		<?php
			...
			echo '<table style="width:100%"><tr><td rowspan=3>';
			...
			<?php
				...
				if($res!=null){
					while ($row = mysqli_fetch_assoc($res)) {
						echo '<span class="clickable" onclick="location.href=\'profile.php?username='.$row['username'].'\'">'.$row['username'].'</span> <small>('.timeDiff($row['id'],round(microtime(true) * 1000)).' ago)</small>';
						if(array_key_exists(("username"), $_SESSION)){
							if($row["username"]==$_SESSION["username"]){
